...
new_df = rare_encoder(df, 0.01)  # --> BURADA KALDIM
...
df.groupby("MoSold")["SalePrice"].mean().sort_values(ascending=False)

df["NEW_Mean_Price_Category"] = np.where(df["MoSold"].isin([9, 11]), "high_price",
                                np.where(df["MoSold"].isin([12, 7, 8, 1, 3]), "mid_price",
                                         np.where(df["MoSold"].isin([10, 2, 6, 5, 4]), "low_price", "")))
df["NEW_Mean_Price_Category"].unique()
df.groupby("NEW_Mean_Price_Category")["SalePrice"].mean().sort_values(ascending=False)
...
#                                              11: "Fall", 12: 'Winter'}) --> hangi mevsimde satildiginin onemi yok.
#                                              rare encoding kisminda buraya dokunulacak.
...
cat_cols, num_cols, cat_but_car = grab_col_names(df) # num_cols dolu geliyor hala
...
# Evin toplam alani + garaj alani --> #150523olusturuldu.
