...
missing_columns = [col for col in df.columns if (df[col].isnull().sum() > 78) & (col!="SalePrice")] #All columns in missing_columns are categorical
#missing_columns = [col for col in df.columns if (df[col].isnull().sum() > 78 and col not in ["SalePrice"])]
...
#replace_with_thresholds(df, num_cols[0:])
...
target_vs_othercols(df_train, "SalePrice", cat_cols)
